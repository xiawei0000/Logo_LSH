...
double ComputeLiangdu(IplImage * img, CvPoint point,CvSize size)
{//计算区域亮度  并归一化
	int i,j;
	long  double liangdu=0;
	uchar * ptr;
	int tmpsum=0;
	int height,width;
	...
	for (j=point.y; j<height ;j++)
	{ 
		ptr=(uchar *)(img->imageData+img->widthStep* j);//得到行指针
		for (i=point.x ;i<width; i++ )
		{
			tmpsum=ptr[i*img->nChannels +0] +ptr[i*img->nChannels +1] +ptr[i*img->nChannels +2];
			liangdu+=tmpsum/3.0;
		}
	}

	liangdu/=( (height-point.y)  * (width -point.x) * 256);
	return liangdu;
...
*/
int inline  fatal_error(char* format, ...)
{
  va_list ap;
